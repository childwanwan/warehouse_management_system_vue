<template>
  <div>
        ...
        <el-menu
          ...
          <el-submenu index="1">
            ...
            <el-menu-item-group>
              ...
              <el-menu-item index="1-9" @click="select1_9">
                系统管理员管理
              </el-menu-item>
            </el-menu-item-group>
          </el-submenu>
    ...
    <!--系统管理员管理-->
    <div id="systemManage" style="display: none">
      <div style="margin-left: 40%;margin-top: 1%;margin-bottom: 1%;font-size: 120%;color: #909399">系统管理员管理</div>

      <!--所有信息-->
      <div id="allsystemManage" style="float: right">
        <el-table
          :data="systemManagerData"
          border
          style="width: 100%">
          <el-table-column
            prop="employeeName"
          ...
          <el-table-column
            ...
            <template slot-scope="scope">　
              <el-button type="primary" style="width: 30%;text-align: center;;height: 25px"
                         @click="updateSystemManager(scope.$index, scope.row)">修改
              </el-button>

              <!--弹框-->
              <el-dialog title="系统管理员信息修改" :visible.sync="systemManagerDialogFormVisible">
                <el-form :model="systemManagerForm">
                  <el-form-item label="管理员名称" :label-width="formLabelWidth">
                    <el-input v-model="systemManagerForm.employeeName" auto-complete="off"></el-input>
                  </el-form-item>
                  <el-form-item label="管理员电话" :label-width="formLabelWidth">
                    <el-input v-model="systemManagerForm.telephone" auto-complete="off"></el-input>
                  </el-form-item>
                  <el-form-item label="管理员地址" :label-width="formLabelWidth">
                    <el-input v-model="systemManagerForm.addr" auto-complete="off"></el-input>
                  </el-form-item>
                  <el-form-item label="管理员状态" :label-width="formLabelWidth">
                    <el-select v-model="systemManagerForm.status" placeholder="请选择管理员状态" style="width: 40%">
                      <el-option v-for="(item,index) in employeeStatus" :value="item.key" :key="item.key">
                        {{item.val}}
                      </el-option>
                    </el-select>
                  </el-form-item>
                ...
                <div slot="footer" class="dialog-footer">
                  <el-button @click="systemManagerDialogFormVisible = false"
                             style="width: 10%;height: 40px;border: 1px solid #dcdfe6;">取 消
                  </el-button>
                  <el-button type="primary" @click="updateSystemManagerToDataBase(scope.$index, scope.row)"
                             style="width: 10%;height: 40px;border: 1px solid #dcdfe6;">确 定
                  </el-button>
                </div>
              </el-dialog>
        ...
        <div style="text-align: center;margin-top: 30px;">
          <el-pagination
            @current-change="systemManagerCurrentChange"
            background
            layout="prev, pager, next"
            :total="this.systemManagerAllPage*10"
          >
          </el-pagination>
        </div>
        ...
        <div data-v-f81d8020="" style="width: 80%;margin-left: 10%;margin-top: 5%">
          <p
            ...
            <span style="color: #909399">+ 添加系统管理员</span>
          </p>
        </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        systemManagerData: [],
        allSystemManager: [],
        systemManagerPageSize: 5,
        systemManagerCount: 0,
        systemManagerCurrentPage: 0,
        systemManagerAllPage: 0,

        //系统管理员修改弹框
        systemManagerDialogFormVisible: false,
        systemManagerForm: {
          telephone: '',
          employeeName: '',
          addr: '',
    ...
    methods: {
      ...
      //系统管理员信息处理
      systemManagerCurrentChange: function (val) {
        //console.log(this.productMessagetableData);
        this.systemManagerCurrentPage = val;
        if (val > 1) {
          ...
          getEmployeesByStatus(data).then((response) => {
            ...
            if (response.retCode === 1) {
              ...
              this.systemManagerData = [];
              if (this.systemManagerCount > this.systemManagerPageSize) {
                if (this.systemManagerCount % this.systemManagerPageSize === 0) {
                  this.systemManagerAllPage = this.systemManagerCount / this.systemManagerPageSize;
                } else {
                  this.systemManagerAllPage = parseInt(this.systemManagerCount / this.systemManagerPageSize) + 1;
                }
                //数组赋值
                for (let i = 0; i < this.systemManagerPageSize; i++) {
                  this.systemManagerData.push(response.data[i]);
                }
              } else {
                this.systemManagerAllPage = 1;
                for (let i = 0; i < this.systemManagerCount; i++) {
                  this.systemManagerData.push(response.data[i]);
                }
              }
      ...
      updateSystemManager: function (index, row) {

        storage.set(SYSTEMMANAGE_KEY, row.id);
        this.systemManagerForm.telephone = this.systemManagerData[index].telephone;
        this.systemManagerForm.employeeName = this.systemManagerData[index].employeeName;
        this.systemManagerForm.addr = this.systemManagerData[index].addr;
        this.systemManagerForm.status = "系统管理员";

        this.systemManagerDialogFormVisible = true;
      },
      updateSystemManagerToDataBase: function (index, row) {
        ...
        this.$confirm('此操作将修改该文件, 是否继续?', '提示', {
          ...
          this.systemManagerForm['id'] = storage.get(SYSTEMMANAGE_KEY);



          if (this.systemManagerForm['status']=='普通职工') {
            this.systemManagerForm['status']=1;
          }else if(this.systemManagerForm['status']=='仓库管理员'){
            this.systemManagerForm['status']=3;
          }else if(this.systemManagerForm['status']=='系统管理员'){
            console.log("hello world");
            this.systemManagerForm['status']=4;
          }else {
            this.systemManagerForm['status'] = 9999;
          }

          console.log(this.systemManagerForm);

          let data = JSON.stringify(this.systemManagerForm);
          updateEmployees(data).then((response) => {
            console.log(response.data);
            if (response.retCode === 1) {
              this.$message({
      ...
      //添加职工
      addSystemManager: function () {
        this.$router.push({
          path: '/addSystemManager'
        });
      },
    ...
    watch: {
      ...
      el_menu_selected: function (val) {
        ...
        if (val === '1-1') {
          let jQueryObject9 = $("#systemManage");
          jQueryObject9.css("display", "none");
          ...
          let jQueryObject9 = $("#systemManage");
          jQueryObject9.css("display", "block");
